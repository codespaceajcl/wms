...
import { getStockReturnReport } from '../../../../../Redux/Action/Admin';
...
export default function StockReturnTab() {
    ...
    const { loading, getStockReturnData } = useSelector((state) => state.inventoryStockReturnData)
    const { loading: stockReturnLoading, getStockReturnFilterData } = useSelector((state) => state.inventoryStockReturnFilterData)
    ...
    React.useEffect(() => {
        ...
        dispatch(getStockReturnReport(formData))

        return () => {
            dispatch({ type: "GET_STOCK_RETURN_FILTER_RESET" })
        }
    }, [])

    React.useEffect(() => {
        if (getStockReturnFilterData?.response?.length > 0) {
            dispatch({ type: "GET_STOCK_RETURN_RESET" })
        }
    }, [getStockReturnFilterData])
    ...
    return (
        <TableContainer>
            <Table aria-label="collapsible table" className='stock_out_table'>
                ...
                {
                    (!loading || !stockReturnLoading) && <>
                        {
                            getStockReturnFilterData?.response ? <>
                                {
                                    getStockReturnFilterData?.response?.length > 0 &&
                                    <TableBody className='stock_out_tab'>
                                        {getStockReturnFilterData?.response?.map((row, i) => (<Row key={i} index={i} row={row} isOpen={openRow === row} onToggle={() => handleRowToggle(row)} />))}
                                    </TableBody>
                                }
                            </> : <>
                                {
                                    getStockReturnData?.response?.length > 0 &&
                                    <TableBody className='stock_out_tab'>
                                        {getStockReturnData?.response?.map((row, i) => (<Row key={i} index={i} row={row} isOpen={openRow === row} onToggle={() => handleRowToggle(row)} />))}
                                    </TableBody>
                                }
                            </>
                        }
            ...
            {(loading || stockReturnLoading) && <Loader />}
        </TableContainer>
    );
}