...
import {
    ...
    getAvailPalletStockIn, getAvailStagesStockIn, getSerialNoExist, stockInRequestDeatils, stockInRequests
} from '../../../../../Redux/Action/Admin';
import { errorNotify } from '../../../../../Util/Toast';
import Loader from '../../../../../Util/Loader';
import { allImages } from '../../../../../Util/Images';

const StockInRequestDetail = () => {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const location = useLocation();
    const indData = location.state && location.state.indData;
    ...
    const [showWarehouse, setShowWarehouse] = useState(null)
    ...
    const { loading: detailLoading, getStockInRequestDeatilsData } = useSelector((state) => state.stockInRequestDetailsData)
    ...
    useEffect(() => {
        ...
        dispatch(stockInRequestDeatils(formData))
    ...
    useEffect(() => {
        if (getStockInRequestDeatilsData?.response && getBusinessWarehouses) {
            ...
            setStockInDetail({
                orderNo: { label: "Request", value: "Request" },
                transactionalNo: indData?.requestNumber,
                vehicleNo: '-',
                recievingDate: indData?.recevingDate,
            })

            const getWarehouse = getBusinessWarehouses?.warehouses.find((w) => w.name === indData.warehouse)

            setShowWarehouse({ value: getWarehouse?.id, label: getWarehouse?.name })
            setGetWarehouseId(getWarehouse?.id)

            setBusinessTypeId(getStockInRequestDeatilsData?.businessType)
            setGetCustomerId(getStockInRequestDeatilsData?.customer)
            dispatch(bussinessTypeCustomer(formData, getStockInRequestDeatilsData?.businessType))
        }

    }, [getBusinessWarehouses, getStockInRequestDeatilsData])

    useEffect(() => {
        if (getStockInRequestDeatilsData?.response && getBusinessCustomers?.response) {
            getStockInRequestDeatilsData?.response?.forEach(item1 => {
                const matchedItem = getBusinessCustomers?.response?.find(item2 => parseInt(item1.partNoId) === parseInt(item2.id));

                if (matchedItem) {
                    matchedItem.qty = item1.quantity;
                    matchedItem.nsn = matchedItem.nsn ? matchedItem.nsn : '-'
                    setShowSterilizeData((prevData) => [...prevData, matchedItem]);
                }
            });
        }
    }, [getStockInRequestDeatilsData, getBusinessCustomers]);
    ...
    const fileHandler = (e, s, i) => {
        if (e.target.files[0]) {
            ...
            readXlsxFile(e.target.files[0]).then((rows, indexs) => {
                ...
                if ((rows.length - 1) !== currentObject.qty) {
                    e.target.value = '';
                    errorNotify("Quantities are not same");
                    return;
                }
    ...
    const submitStockInHandler = () => {
        ...
        dispatch(createStockIn(formData))
    ...
    const modal2 = <Modal centered show={stockInShow} onHide={() => navigate('/wms/dashboard')} className='success' style={{ backgroundColor: '#00000040' }}>
        <Modal.Body>
            ...
            <div>
                ...
                <h2 style={{ textAlign: "center" }}>Stock In Created!</h2>
    ...
    return (
        <div>
            ...
            <div className='material_main' style={{ padding: "25px 0" }}>
                <h5> <BsArrowLeftShort onClick={backHandler} style={{ left: "10px" }} /> Stock In Request
                    <div className='create serial_generate' onClick={serialModal}>
                        <img src={allImages.serial_icon} alt='' /> Generate Serial No</div>
                </h5>

                {
                    (businessLoading || customerLoading || detailLoading) ? <Loader /> :
                        <>
                            <Row style={{ padding: "0 20px" }} className='mt-2 align-items-center'>
                                <Col md={5} className='mt-2'>
                                    ...
                                    <Select options={options} value={stockInDetail.orderNo} onChange={(d) => setStockInDetail({
                                        ...stockInDetail,
                                        orderNo: d
                                    })} className='disabled-select' placeholder="Select" styles={materialColorStyles} isDisabled />
                                </Col>
                                ...
                                <Col md={5} className='mt-2'>
                                    ...
                                    <Select className='disabled-select' isDisabled isLoading={businessLoading} value={{ value: businessTypeId, label: businessTypeId }} options={businessTypeOption} onChange={getCustomerHandler} placeholder="Select Company" styles={materialColorStyles} />
                                </Col>
                                <Col md={5} className='mt-2'>
                                    <label className='react_select_label'>Warehouse <span>*</span></label>
                                    <Select className='disabled-select' isDisabled isLoading={businessLoading} value={showWarehouse} options={warehouseOption} onChange={warehouseHandler} placeholder="Select Warehouse" styles={materialColorStyles} />
                                </Col>
                                <Col md={5} className='mt-2'>
                                    <label className='react_select_label'>Customer <span>*</span></label>
                                    <Select className='disabled-select' isDisabled isLoading={customerLoading} value={{ value: customerId, label: customerId }} options={customersOptions} onChange={customerHandler} placeholder="Select" styles={materialColorStyles} />
                                </Col>
                            </Row>
                                ...
                                (warehouseId && customerId?.length > 0) && <>
                                    <div className='shipment_stock_main'>
                                        ...
                                        <Row className='my-2 mx-2 align-items-center'>
                                            <Col md={8}>
                                                <Select isDisabled isLoading={customerLoading} options={partNoOtion} onChange={searchPartHandler} value={selectedBusiness} placeholder="Search Part No/Noms/NSN" styles={partColorStyles} className='react_select_inhouse stock' />
                                            </Col>
                                            <Col md={4}>
                                                            ...
                                                            showSterilizeData?.map((s, i) => {
                                                                return (
                                                                    <tr key={i}>
                                                                        ...
                                                                        <td>{s.nsn}</td>
                                                                        <td key={`fileInput-${i}`}> {serialLoading ? <Loader /> : s.fileName ?
                                                                            <span style={{ color: "#0d6efd", fontWeight: "500" }}>{s.fileName}</span> :
                                                                            <input type='file' types={["xlsx"]} className='sterilize' onChange={(e) => fileHandler(e, s, i)} />} </td>
                                                                        <td>{s?.qty ? s.qty : 0}</td>
...
export default StockInRequestDetail