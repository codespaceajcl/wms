...
import {
    businessTypeWarehouse, bussinessTypeCustomer, createStockIn, createStockTransfer, generateSerialNo, getAvailLocationStockIn,
...
const TransferredStock = () => {
    ...
    const { loading: createLoading, postStockTransfer } = useSelector((state) => state.postStockTransferApi)
    ...
    useEffect(() => {
        if (postStockTransfer?.response === "success") {
            setStockInShow(true)
            setStockInSgi(postStockTransfer?.sgi)
            dispatch({ type: "CREATE_STOCK_IN_RESET" })
        }

        else if (postStockTransfer?.response === "noAccess") {
            errorNotify(postStockTransfer?.response)
            dispatch({ type: "CREATE_STOCK_IN_RESET" })
        }
    }, [postStockTransfer])
    ...
    const submitStockInHandler = () => {
        ...
        dispatch(createStockTransfer(formData))
    ...
    const modal2 = <Modal centered show={stockInShow} onHide={() => navigate('/wms/dashboard')} className='success' style={{ backgroundColor: '#00000040' }}>
        <Modal.Body>
            ...
            <div>
                ...
                <h2 style={{ textAlign: "center" }}>Stock Transferred Successfully!</h2>
